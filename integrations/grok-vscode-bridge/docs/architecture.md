...
That description applies to the default `grok.connectionMode: internal`. In the
opt-in `external` mode, the extension does not spawn Grok. `external-broker.ts`
connects to the separately deployed OpenClaw/Grok broker over an authenticated
Unix socket. It honors an absolute `XDG_RUNTIME_DIR` and falls back to Linux's
`/run/user/<uid>` when the VS Code extension host omits that variable.
`external-sidebar.ts` renders the broker's filtered, cursor-addressed
event journal. Codex and VS Code therefore attach to the same broker-owned ACP
process and disposable worktree. External steering is disabled by default and
must be enabled separately with `grok.externalSteeringEnabled`.
...
External mode uses a separate path:

```text
VS Code external webview ─► extension host ─► authenticated Unix socket ─► broker ─► grok agent stdio
Codex MCP client ────────────────────────────────────────────────────────┘
```

The external client accepts only filtered state/message/tool/file/test/
permission/completion/error events; raw thoughts and environments are outside
the contract.
