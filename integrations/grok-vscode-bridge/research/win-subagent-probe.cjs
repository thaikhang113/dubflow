// Diagnostic: capture the REAL spawn_subagent tool_call payload on native
// Windows grok — full title/kind/rawInput — so the subagent card's matcher +
// label can be confirmed against ground truth (was TODO in CLAUDE.md).
const { spawn } = require("node:child_process");
const os = require("node:os");
const path = require("node:path");
const fs = require("node:fs");
const GROK = path.join(process.env.USERPROFILE || os.homedir(), ".grok", "bin", "grok.exe");
const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "grok-winsub-"));
...
const p = spawn(GROK, ["--always-approve", "agent", "stdio"], { cwd, env: process.env });
...
function handle(m) {
  if (m.id != null && m.method == null) {
    ...
    else if (m.id === newId) { sessionId = m.result.sessionId; console.log("session", sessionId); promptId = send("session/prompt", { sessionId, prompt: [{ type: "text", text: "Spawn a subagent to investigate this codebase and report what add() does. Use a subagent for the investigation." }] }); console.log("sent subagent prompt"); }
  ...
  if (m.method === "session/update") {
    ...
    if (t === "tool_call" || t === "tool_call_update") {
      ...
      if (/subagent|agent_type|spawn|delegat|task/i.test(blob)) {
        console.log("\n=== " + t + " (subagent-ish) ===");
        console.log("  toolCallId:", u.toolCallId);
        console.log("  title:", JSON.stringify(u.title));
        console.log("  kind:", u.kind, "status:", u.status);
        if (u.rawInput) console.log("  rawInput:", JSON.stringify(u.rawInput).slice(0, 600));
...
setTimeout(() => { console.log("TIMEOUT"); p.kill(); process.exit(0); }, 180000);
