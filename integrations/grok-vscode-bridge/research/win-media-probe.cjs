// Diagnostic: dump every tool_call / tool_call_update for an /imagine prompt on
// native Windows grok, to see why extractGeneratedMediaPaths misses the result.
const { spawn } = require("node:child_process");
const os = require("node:os");
const path = require("node:path");
const fs = require("node:fs");
const GROK = path.join(process.env.USERPROFILE || os.homedir(), ".grok", "bin", "grok.exe");
const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "grok-winmedia-"));
...
function handle(m) {
  if (m.id != null && m.method == null) {
    ...
    else if (m.id === newId) { sessionId = m.result.sessionId; console.log("session", sessionId); promptId = send("session/prompt", { sessionId, prompt: [{ type: "text", text: "/imagine a small red cube on a white background" }] }); console.log("sent /imagine"); }
  ...
  if (m.method === "session/update") {
    ...
    if (t === "tool_call" || t === "tool_call_update") {
      ...
      if (u.content) console.log("  content:", JSON.stringify(u.content).slice(0, 800));
    }
    return;
  }
...
setTimeout(() => { console.log("TIMEOUT"); p.kill(); process.exit(0); }, 120000);
